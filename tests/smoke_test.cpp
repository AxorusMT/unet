...
int main() {
    constexpr std::uint16_t kPort = 34567;
    ...
    const bool connected = pump_until(std::chrono::milliseconds(2000), [&]() {
        server.service();
        client.service();
    ...
    const auto payload = unet::to_bytes("ping");
    unet::SendOptions options{};
    options.channel = 0;
    options.delivery = unet::Delivery::ReliableOrdered;
    auto sent = client.send(client_peer, payload, options);
        ...
        while (auto event = server.poll_event()) {
            if (event->type == unet::Event::Type::Message) {
                ...
                if (text == "ping") {
                    ...
                    const auto pong = unet::to_bytes("pong");
                    (void)server.send(server_peer, pong, options);
                }
            }
        }
        while (auto event = client.poll_event()) {
